...
def user_sessions(sessions):
    # combinebyKey causes too much shuffle over the network
    """
    ...
    # reduceByKey causes less shuffle
    sessions = sessions.reduceByKey(lambda merger, sess: merger + sess)
...
""" avoids datetime is not JSON object serializable error """
def makeJSON(data):
    data["date"] = data["date"].strftime("%Y/%m/%d, %H:%M:%S")
    return data

""" cast a feature column before output  """
def castVariables(data):
    data["avg_discount"] = float(data["avg_discount"])  if data["avg_discount"] else 0.0 
    data["avg_price_cart_item"] = float(data["avg_price_cart_item"])  if data["avg_price_cart_item"] else 0.0 
    data["avg_price_viewed_item"] = float(data["avg_price_viewed_item"]) if data["avg_price_viewed_item"] else 0.0
    data["buy_to_detail_rate"] = float(data["buy_to_detail_rate"])  if data["buy_to_detail_rate"] else 0.0
    data["cart_amount"] = float(data["cart_amount"])  if data["cart_amount"] else 0.0      
    data["cartadd_to_detail_rate"] = float(data["cartadd_to_detail_rate"])  if data["cartadd_to_detail_rate"] else 0.0   
    data["current_price_cart_item"] = float(data["current_price_cart_item"])   if data["current_price_cart_item"] else 0.0                                                       
    data["current_price_viewed_item"] = float(data["current_price_viewed_item"])   if data["current_price_viewed_item"] else 0.0   
    data["current_time_diff_between_purchases"] = float(data["current_time_diff_between_purchases"]) if data["current_time_diff_between_purchases"] else 0.0   
    data["discount_avg"] = float(data["discount_avg"]) if data["discount_avg"] else 0.0   
    data["discount_rate_viewed_item"] = float(data["discount_rate_viewed_item"]) if data["discount_rate_viewed_item"] else 0.0   
    data["last2_purchase_value"] = float(data["last2_purchase_value"]) if data["last2_purchase_value"] else 0.0   
    data["last_avg_price_viewed_item"] = float(data["last_avg_price_viewed_item"]) if data["last_avg_price_viewed_item"] else 0.0   
    data["last_discount_rate_viewed_item"] = float(data["last_discount_rate_viewed_item"]) if data["last_discount_rate_viewed_item"] else 0.0   
    data["last_duration_bf_buy"] = float(data["last_duration_bf_buy"]) if data["last_duration_bf_buy"] else 0.0   
    data["discount_avg"] = float(data["discount_avg"]) if data["discount_avg"] else 0.0
    data["last_max_price_viewed_item"] = float(data["last_max_price_viewed_item"]) if data["last_max_price_viewed_item"] else 0.0
    data["last_min_price_viewed_item"] = float(data["last_min_price_viewed_item"]) if data["last_min_price_viewed_item"] else 0.0
    data["last_purchase_value"] = float(data["last_purchase_value"]) if data["last_purchase_value"] else 0.0
    data["last_session_length"] = float(data["last_session_length"]) if data["last_session_length"] else 0.0
    data["last_total_price_viewed_item"] = float(data["last_total_price_viewed_item"]) if data["last_total_price_viewed_item"] else 0.0
    data["max_price_cart_item"] = float(data["max_price_cart_item"]) if data["max_price_cart_item"] else 0.0
    data["max_time_between_purchases"] = float(data["max_time_between_purchases"]) if data["max_time_between_purchases"] else 0.0
    data["max_value_purchases"] = float(data["max_value_purchases"]) if data["max_value_purchases"] else 0.0
    data["mean_duration_bf_buy"] = float(data["mean_duration_bf_buy"])  if data["mean_duration_bf_buy"] else 0.0 
    data["mean_session_length"] = float(data["mean_session_length"])  if data["mean_session_length"] else 0.0                                
    data["mean_success_rate"] = float(data["mean_success_rate"])   if data["mean_success_rate"] else 0.0 
    data["mean_time_between_purchases"] = float(data["mean_time_between_purchases"])   if data["mean_time_between_purchases"] else 0.0 
    data["mean_value_purchases"] = float(data["mean_value_purchases"])   if data["mean_value_purchases"] else 0.0 
    data["median_duration_bf_buy"] = float(data["median_duration_bf_buy"])   if data["median_duration_bf_buy"] else 0.0 
    data["median_num_pageview_bf_buy"] = float(data["median_num_pageview_bf_buy"])   if data["median_num_pageview_bf_buy"] else 0.0 
    data["median_num_sessions_bf_buy"] = float(data["median_num_sessions_bf_buy"])   if data["median_num_sessions_bf_buy"] else 0.0 
    data["median_time_between_purchases"] = float(data["median_time_between_purchases"])   if data["median_time_between_purchases"] else 0.0 
    data["median_value_purchases"] = float(data["median_value_purchases"]) if data["median_value_purchases"] else 0.0  
    data["mex_duration_bf_buy"] = float(data["mex_duration_bf_buy"]) if data["mex_duration_bf_buy"] else 0.0 
    data["min_price_cart_item"] = float(data["min_price_cart_item"]) if data["min_price_cart_item"] else 0.0 
    data["min_price_viewed_item"] = float(data["min_price_viewed_item"]) if data["min_price_viewed_item"] else 0.0
    data["max_price_viewed_item"] = float(data["max_price_viewed_item"]) if data["max_price_viewed_item"] else 0.0  
    data["money_spent_in_last_30_days"] = float(data["money_spent_in_last_30_days"]) if data["money_spent_in_last_30_days"] else 0.0 
    data["money_spent_in_last_7_days"] = float(data["money_spent_in_last_7_days"]) if data["money_spent_in_last_7_days"] else 0.0 
    data["session_length"] = float(data["session_length"]) if data["session_length"] else 0.0 
    data["sessionid"] = str(data["sessionid"]) if data["sessionid"] else ""
    data["std_time_between_purchases"] = float(data["std_time_between_purchases"]) if data["std_time_between_purchases"] else 0.0 
    data["time_elapsed_last_cart_add"] = float(data["time_elapsed_last_cart_add"]) if data["time_elapsed_last_cart_add"] else 0.0 
    data["time_elapsed_last_page"] = float(data["time_elapsed_last_page"]) if data["time_elapsed_last_page"] else 0.0 
    data["time_since_last_purchase"] = float(data["time_since_last_purchase"]) if data["time_since_last_purchase"] else 0.0 
    data["userid"] = str(data["userid"]) if data["userid"] else ""
    data["total_price_viewed_item"] = float(data["total_price_viewed_item"]) if data["total_price_viewed_item"] else 0.0
    data["last_first_page"] = hash(data["last_first_page"]) if data["last_first_page"] else -1
    data["last_last_page"] = hash(data["last_last_page"]) if data["last_last_page"] else -1
    data["last_time_since"] = float(data["last_time_since"]) if data["last_time_since"] else -1.0
    return data
...
def main():
    ...
    *** cast string date to timestamp
    ...
    """ don't run if inputFiles are already /data/insider/partitioned  """
    # sessions = sessions.repartition(1000)

    sessions.cache().count()
    ...
    data = data.map(lambda features: dict(ChainMap(*features))
                                         ).map(lambda features: castVariables(features)
                                                ).map(lambda features: Row(**features)) # .map(lambda features: makeJSON(features)).map(lambda features: dumps(features))

    # output = spark.read.json(data)
    data.count()
    output = spark.createDataFrame(data)

    """ use this if inputfiles are /data/insider/partitioned """
    output.repartition(1).write.csv("/home/ubuntu/spark-3.0.0-bin-hadoop2.7/outputInsider", header=True)

    # to save as multiple partitions
    """
    output.repartition(1000).write.csv("/home/ubuntu/spark-3.0.0-bin-hadoop2.7/outputInsider", header=True)
    """
